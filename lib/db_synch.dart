...
*/
  ...
  Future<Database> initDB() async {
    ...
    List<Map> list = await db.rawQuery("""
      select (select value from info where name = 'login') login,
             ...
             (select value from info where name = 'firebase_token') firebase_token
    """);
    login = list[0]['login'];
    ...
    if (firebaseToken == null) {
      firebaseToken = list[0]['firebase_token'];
    } else {
      await db.execute("REPLACE INTO info (name, value) VALUES ('firebase_token', '$firebaseToken')");
    }
  ...
  Future<String> fillDB() async {
    ...
    do {
      ...
      try {
        print("url = $url i = $i");
        print("RApi client_id=$clientId,token=$token, firebaseToken = $firebaseToken");
        response = await httpClient.get(url,
          headers: {"Authorization": "RApi client_id=$clientId,token=$token", "FirebaseToken": firebaseToken}
        );
      } catch(exception) {
        return 'Сервер $server недоступен!\n$exception';
      }
